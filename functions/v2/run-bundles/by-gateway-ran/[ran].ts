// functions/v2/run-bundles/by-gateway-ran/[ran].ts
//
// GET /v2/run-bundles/by-gateway-ran/{ran} — PUBLIC.
...
const ID_RE = /^RAN-\d{12}$/;
const KEY_PREFIX = "run-bundles-by-gateway-ran:";
...
export const onRequestGet: PagesFunction<Env> = async ({ env, params }) => {
  const id = (params as { ran: string }).ran;
  if (!ID_RE.test(id)) {
    return json({ error: "Invalid RAN format" }, 400);
  }
